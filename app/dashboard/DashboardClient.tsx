        ...
        <div className="px-6 py-3 border-b border-zinc-200 bg-white flex items-center justify-between flex-shrink-0">
          <div>
            <p className="text-sm font-semibold text-zinc-900">
              {selectedEmail ? `Context: ${parseSender(selectedEmail.from)} — ${selectedEmail.subject}` : "MailMind AI"}
            </p>
            <p className="text-xs text-zinc-400">
              {selectedEmail ? "Asking about this email" : `${emails.length} emails loaded · Ask anything`}
            </p>
          </div>
          {selectedEmail && (
            <button
